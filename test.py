...
class MusicPlayer:
    def __init__(self):
        ...
        # 显示时间标签
        self.time_label = tk.Label(self.window, text="00:00 / 00:00", font=("Helvetica", 12))
        self.time_label.grid(row=5, column=0, columnspan=2, pady=5)

        # 进度条
        self.progress = ttk.Scale(self.window, from_=0, to=100, orient="horizontal", command=self.update_progress)
    ...
    def update_time_label(self, current_time):
        ...
        total_minutes, total_seconds = divmod(int(getattr(self, 'song_length', 0)), 60)
